...
func getAzureMachinePoolWithScaleSetPriority(name string, scaleSetPrioirty string) *infrav1.AzureManagedMachinePool {
	managedPool := getAzureMachinePool(name, infrav1.NodePoolModeUser)
	managedPool.Spec.ScaleSetPriority = to.StringPtr(scaleSetPrioirty)
	return managedPool
}
